...
import {
  CheckCircle2, ArrowRight, LayoutGrid, Home, Ruler, Building,
...
const included = [
  '2D floor plans to RICS measuring standards',
  'Accurate room dimensions and total floor area (sq ft / sq m)',
  'Compass orientation and furniture layout on request',
  'High-resolution files ready for property listings',
  'Fast turnaround — most plans delivered within 48 hours',
];

const whoNeeds = [
  { title: 'Estate Agents', desc: 'Listing-ready floor plans that help properties sell and let faster.', Icon: Home },
  { title: 'Landlords', desc: 'Clear layouts for marketing lettings and satisfying tenant enquiries.', Icon: Building },
  { title: 'Homeowners', desc: 'Planning a renovation or extension? Start with an accurate as-built plan.', Icon: Ruler },
];

const formats = [
  { title: '2D Standard', desc: 'Clean line-drawn plan with room labels and dimensions — our most popular option.' },
  { title: '3D Rendered', desc: 'A visual, furnished 3D layout that helps buyers picture the space instantly.' },
  { title: 'Planning-Grade', desc: 'Precision plans formatted to meet local council planning application requirements.' },
];

const steps = [
  { title: 'Book a Survey', desc: 'Arrange a convenient time for our surveyor to measure your property.' },
  { title: 'On-Site Measuring', desc: 'Laser measurement of every room, typically 20-40 minutes.' },
  { title: 'Plan Delivered', desc: 'Your finished floor plan lands in your inbox, ready to use.' },
];

const stats = [
  { value: 2000, suffix: '+', label: 'Floor Plans Delivered', Icon: FileCheck },
  { value: 48, suffix: 'hrs', label: 'Average Turnaround', Icon: Clock },
  { value: 400, suffix: '+', label: 'Estate Agents Served', Icon: Users },
  { value: 100, suffix: '%', label: 'RICS Standard', Icon: Ruler },
];

const faqs = [
  { q: 'How long does a floor plan take?', a: 'Most floor plans are delivered within 24-48 hours of the site visit.' },
  { q: 'Can you do 3D floor plans?', a: 'Yes, we offer 3D and furnished floor plans as an add-on for an enhanced listing.' },
  { q: 'Do you cover commercial properties?', a: 'Absolutely — we produce floor plans for residential, commercial and mixed-use properties.' },
  { q: 'What file formats do you provide?', a: 'High-resolution PNG and PDF as standard, with editable CAD files available on request.' },
  { q: 'Can floor plans be used for planning applications?', a: 'Yes — our planning-grade plans are formatted to meet local council submission requirements.' },
];
...
<section className="relative bg-navy pt-32 pb-0 overflow-hidden min-h-[92vh] flex flex-col">
  <div className="absolute inset-0 z-0">
    <img
      src="https://images.unsplash.com/photo-1608303588026-884930af2559?fm=jpg&q=80&w=2600&auto=format&fit=crop"
    ...
    <div className="grid lg:grid-cols-[1.15fr_0.85fr] gap-14 items-center w-full">
      <div>
        ...
        <h1 className={`font-sans text-4xl sm:text-5xl lg:text-[3.6rem] font-extrabold text-white mb-6 leading-tight transition-all duration-700 delay-100 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
          ...
          <br />
          <span className="text-gold">That Sell Properties</span>
        </h1>
        <p className={`text-white/85 text-lg max-w-xl mb-9 leading-relaxed transition-all duration-700 delay-200 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
          Professionally measured, RICS-standard floor plans for sales listings, lettings and
          planning applications across Manchester.
        </p>
      ...
      <div className={`hidden lg:flex justify-center transition-all duration-1000 delay-300 ${mounted ? 'opacity-100 scale-100' : 'opacity-0 scale-90'}`}>
        <div className="w-56 h-56 rounded-full border-2 border-gold/60 flex items-center justify-center bg-white/5 animate-float">
          <LayoutGrid size={90} strokeWidth={1.3} className="text-gold" />
        </div>
      </div>
    </div>
  </div>
  ...
  <div className="relative z-10 max-w-content mx-auto px-6 pb-14">
    ...
    >
      {[
        { label: 'RICS Standard', desc: 'Precise, industry-compliant measuring', Icon: Ruler },
        { label: '2D & 3D Options', desc: 'Furnished renders available on request', Icon: LayoutGrid },
        { label: '48hr Delivery', desc: 'High-res files ready for listings', Icon: Home },
      ].map(({ label, desc, Icon }) => (
          ...
          <div className="max-w-content mx-auto px-6 grid lg:grid-cols-2 gap-16 items-center">
            <div>
              ...
              <h2 className="font-sans text-3xl sm:text-4xl font-extrabold text-navy leading-tight mb-6">
                Listing-Ready Plans, Every Time
              </h2>
            ...
            <div className="bg-navy-dark rounded-2xl p-10">
              <LayoutGrid size={32} className="text-gold mb-4" />
              <h3 className="text-white font-bold text-2xl mb-3">Bundle &amp; Save</h3>
              <p className="text-white/60 mb-6 leading-relaxed">
                Combine your floor plan with an EPC or EICR booking and save on our full compliance package.
              </p>
              <Link href="/services/compliance" className="inline-flex items-center gap-2 bg-gold text-navy font-bold text-sm tracking-wide px-6 py-3.5 rounded hover:bg-gold-light transition-colors duration-200">
                VIEW COMPLIANCE PACKAGE <ArrowRight size={16} />
              </Link>
            </div>
          </div>
        </section>
        ...
        <section className="bg-white py-24 border-t border-navy/5">
          <div className="max-w-content mx-auto px-6">
            <div className="text-center max-w-xl mx-auto mb-16">
              <p className="text-gold-dark text-sm font-bold tracking-[0.25em] mb-3">CHOOSE YOUR FORMAT</p>
              <h2 className="font-sans text-3xl sm:text-4xl font-extrabold text-navy">Plans Built Around Your Needs</h2>
            </div>
            <div className="grid sm:grid-cols-3 gap-8">
              {formats.map((f) => (
                <div key={f.title} className="border border-navy/10 rounded-xl p-7">
                  <h3 className="text-navy font-bold text-lg mb-3">{f.title}</h3>
        ...
        <section className="bg-navy-dark py-24">
          <div className="max-w-content mx-auto px-6">
            <div className="text-center max-w-xl mx-auto mb-16">
              <p className="text-gold text-sm font-bold tracking-[0.3em] mb-3">WHO NEEDS A FLOOR PLAN</p>
              <h2 className="font-sans text-3xl sm:text-4xl font-extrabold text-white">Is This Service For You?</h2>
            </div>
            <div className="grid sm:grid-cols-3 gap-8">
                ...
                <section className="bg-white py-24">
                          <div className="max-w-content mx-auto px-6 max-w-3xl">
                            <div className="text-center mb-14">
                              ...
                              <h2 className="font-sans text-3xl sm:text-4xl font-extrabold text-navy">Floor Plan Questions Answered</h2>
                            </div>
        ...
        <section className="bg-navy py-20">
          <div className="max-w-content mx-auto px-6 text-center">
            <h2 className="font-sans text-3xl sm:text-4xl font-extrabold text-white mb-4">
              Get a Floor Plan That Does Your Listing Justice
            </h2>
            <p className="text-white/60 max-w-xl mx-auto mb-8">
              Book a measuring survey and have your plan ready within 48 hours.
            </p>
            <Link href="/booking" className="inline-flex items-center gap-2 bg-gold text-navy font-bold text-sm tracking-wide px-8 py-4 rounded-md hover:bg-gold-light transition-colors duration-300">
              BOOK AN APPOINTMENT <ArrowRight size={16} />
            </Link>
